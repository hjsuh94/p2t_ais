...
class CompressionConv(nn.Module):
    ...
    def __init__(self, z, a):
        ...
        self.compression_conv = nn.Sequential(
            nn.Conv2d(1, 32, 4, stride=2), # 32 x 64 x 64
            nn.ReLU(),
            nn.Conv2d(32, 64, 4, stride=2), # 64 x 32 x 32
            nn.ReLU(),
            nn.Conv2d(64, 128, 4, stride=2), # 128 x 16 x 16
            nn.ReLU(),
            nn.Conv2d(128, 256, 4, stride=2), # 256 x 8 x 8
            nn.ReLU(),
        )
...
class DeCompressionConv(nn.Module):
    def __init__(self, z, a):
        super(DeCompressionConv, self).__init__()
